...
enum Variant select_variant();
void run(struct Rules rules, struct GameState game_state, struct GraphicsContext graphics_context);

int main() {
    enum Variant variant = select_variant();
    ...
    run(rules, game_state, graphics_context);
...
enum Variant select_variant() {
    ...
    variant = STANDARD_10X10_CHESS;          //
    //variant = LONG_RANGE_CHESS;              //
    //variant = KING_MARCH_CHESS;
    //variant = FOUR_D_4X4X4X4_V1_CHESS;       //
    //variant = GRAVITY_CHESS;
    //variant = TWO_MOVES_CHESS;
    //variant = TEN_MOVES_CHESS;
    //variant = MOVE_TO_ANY_SQUARE_CHESS;
    
    //variant = STANDARD_CHESS;
    //variant = ANYTHING_CAN_PROMOTE_CHESS;
    //variant = STANDARD_24X24_CHESS;
    //variant = SPARSE_CHESS;
    //variant = RANK_SEVEN_AND_EIGHT_CHESS;
    //variant = MORE_PAWNS_CHESS;
    //variant = MORE_PAWNS_GRAVITY_CHESS;

    //variant = WRAPPING_8X14_CHESS;
    //variant = WRAPPING_10X10_CHESS;
    //variant = WRAPPING_12X12_CHESS;          //
    //variant = THREE_D_5X5X5_CHESS;           //
    //variant = FOUR_D_3X3X3X3_V1_CHESS;
    //variant = FOUR_D_3X3X3X3_V2_CHESS;       //
    //variant = FOUR_D_3X3X3X3_V3_CHESS;       //
    //variant = FOUR_D_3X3X3X3_V4_CHESS;
    //variant = FOUR_D_4X4X4X4_V2_CHESS;
    //variant = FOUR_D_8X8X8X8_V2_CHESS;
    //variant = SIX_D_2X2X2X2X2X2_CHESS;
    //variant = SIX_D_3X3X3X3X3X3_CHESS;
    return variant;
}
            ...
            switch (event.type) {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    ...
                    int square_index = get_square_index_at_coordinates(event.button.x, event.button.y, 
                            graphics_context.graphics_board, graphics_context.graphics_board_length, graphics_context.square_width
                            );
                    if (square_index == -1 || square_index == selected_square_index) {  
                        // outside board -> reset highlighted squares
                        selected_square_index = -1;
                        moves[0].destination_square = -1;   // todo: can we skip doing this?
                        diagonal_pawn_moves[0].destination_square = -1;
                    } else if (selected_square_index == -1) {   
                        // on board -> highlight squares
                        selected_square_index = square_index;
                        get_moves(moves, diagonal_pawn_moves, square_index, &game_state, &rules);   // todo: can we skip doing this here?
                    } else {                                    
                        // on board and squares already highlighted -> make move or highlight new square
